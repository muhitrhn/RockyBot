...
module.exports = {
  ...
  async execute(client, interaction) {
    try {
      ...
      if (!interaction.guild.me.permissionsIn(interaction.channel).has('KICK_MEMBERS')) {
        //PermsCheck: missing bot perms
        const ifBot = 1
        return client.base.get('check').missingPerms(client, interaction, missingPerms, ifBot)
      } 
      else if (!interaction.member.permissionsIn(interaction.channel).has('KICK_MEMBERS')  && !client.ownerID.includes(interaction.author.id)) {
      ...
      if (mentioned.permissions.has('MANAGE_MESSAGES')) {
        embed.setTitle(`${client.emotes.warn}  Nie mogę wyrzucić...`)
      ...
      collector.on('collect', async buttonClick => {
        ...
        else if (buttonClick.customId === 'kick') {
