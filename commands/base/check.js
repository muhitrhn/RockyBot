...
module.exports = {
  ...
  async member(client, message, args) {
    let mentionMember = await message.guild.member(args[0]), mentioned
    if (message.mentions.members.size) {
  ...
  async missingPerms(client, message, args, pf, cmd, reaction, missingPerms, ifBot) {
    try {
