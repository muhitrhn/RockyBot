...
module.exports = {
  name: "jail",
  aliases: ["aj"],
  description: 'Zamknij kogoś w więzieniu',
  category: 'avatar',
  utilisation: '{prefix}aj <wzmianka>',
  async execute(client, message, args, pf, cmd) {
    ...
    try {
      ...
      if (!message.mentions.members.size) {
        ...
        let doneAv
        try {
          doneAv = await new DIG.Jail().getImage(`${message.author.displayAvatarURL({ dynamic: false, format: "png" })}?size=4096`)
        } catch (err) {
        ...
        //ATTACH AV
        try {
          const attachment = new MessageAttachment(doneAv, "inJail.png")
      ...
      //SOMEONE MENTIONED
        ...
        let doneAv
        try {
          doneAv = await new DIG.Jail().getImage(`${mentioned.displayAvatarURL({ dynamic: false, format: "png" })}?size=4096`)
        } catch (err) {
          errEmbed.setDescription(`${client.emotes.grverify} Tworzenie wiadomości\n${client.emotes.grverify} Wzmianka: ${mentioned}\n${client.emotes.x} Przerabianie avataru`)
          reaction.edit(errEmbed)
          return;
