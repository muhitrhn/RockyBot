  ...
  async execute(client, message, args, pf, cmd) {
    ...
    try {
      ...
      if (!message.mentions.members.size) {
        ...
        let doneAv
        try {
          doneAv = await new DIG.Invert().getImage(`${message.author.avatarURL({ dynamic: false, format: "png" })}?size=4096`)
        } catch (err) {
      ...
      //SOMEONE MENTIONED
        ...
        let doneAv
        try {
          doneAv = await new DIG.Invert().getImage(`${mentioned.avatarURL({ dynamic: false, format: "png" })}?size=4096`)
        } catch (err) {
          errEmbed.setDescription(`${client.emotes.grverify} Tworzenie wiadomości\n${client.emotes.grverify} Wzmianka: ${mentioned}\n${client.emotes.x} Przerabianie avataru`)
          reaction.edit(errEmbed)
          return;
