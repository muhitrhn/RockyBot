...
module.exports = {
  name: "invert",
  aliases: ["ai"],
  description: 'Odwróć kolory czyjegoś avataru',
  category: 'avatar',
  utilisation: '{prefix}ai <wzmianka>',
  async execute(client, message, args, pf, cmd) {
    ...
    try {
      ...
      if (!message.mentions.members.size) {
        ...
        let doneAv
        try {
          doneAv = await new DIG.Invert().getImage(`${message.author.displayAvatarURL({ dynamic: false, format: "png" })}?size=4096`)
        } catch (err) {
        ...
        //ATTACH AV
        try {
          const attachment = new MessageAttachment(doneAv, "inverted.png")
      ...
      //SOMEONE MENTIONED
        ...
        let doneAv
        try {
          doneAv = await new DIG.Invert().getImage(`${mentioned.displayAvatarURL({ dynamic: false, format: "png" })}?size=4096`)
        } catch (err) {
          errEmbed.setDescription(`${client.emotes.grverify} Tworzenie wiadomości\n${client.emotes.grverify} Wzmianka: ${mentioned}\n${client.emotes.x} Przerabianie avataru`)
          reaction.edit(errEmbed)
          return;
