...
if (!message.mentions.members.size) {
    ...
    //AVATAR
      ...
      new DIG.Delete().getImage(`${message.author.displayAvatarURL({ dynamic: false, format: "png" })}?size=4096`).then(async image => {
...
//SOMEONE MENTIONED
     ...
     if (!mentioned.avatarURL()) {
      ...
      //AVATAR
        ...
        embed.setDescription(`${client.emotes.system}  Użyto komendy: **${message.content}**`)
        new DIG.Delete().getImage(`${mentioned.displayAvatarURL({ dynamic: false, format: "png" })}?size=4096`).then(async image => {
          let attachment = new MessageAttachment(image, "deleted.png")
          await message.lineReplyNoMention({embed, files: [attachment]})
          if (reaction) await reaction.remove()
        })
