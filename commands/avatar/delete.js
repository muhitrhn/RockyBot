...
module.exports = {
    ...
    category: 'avatar',
    utilisation: '{prefix}delete <wzmianka>',
   ...
   if (!message.mentions.members.size) {
        ...
        //CREATE IMAGE
       try {
      doneAv = await new DIG.Delete().getImage(`${message.author.displayAvatarURL({ dynamic: false, format: "png" })}?size=4096`)
      } catch (error) {
      ...
      //ATTACH AV
      try {
        let attachment = new MessageAttachment(doneAv, "deleted.png")
   ...
   //SOMEONE MENTIONED
        ...
        //CREATE IMAGE
       try {
      doneAv = await new DIG.Delete().getImage(`${mentioned.displayAvatarURL({ dynamic: false, format: "png" })}?size=4096`)
      } catch (error) {
      errorEmbed.setDescription(`${client.emotes.grverify} Tworzenie wiadomości\n${client.emotes.grverify} Wzmianka: ${mentioned}\n${client.emotes.x} Przerabianie avataru`)
      reaction.edit(errorEmbed)
      return;
