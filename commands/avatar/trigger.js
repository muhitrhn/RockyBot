   ...
   const reaction = await message.lineReplyNoMention(reactionEmbed)
    ...
    try {
      ...
      if (!message.mentions.members.size) {
        ...
        let doneAv
        try {
          doneAv = await new DIG.Triggered().getImage(`${message.author.avatarURL({ dynamic: false, format: "png" })}?size=4096`)
        } catch (err) {
      ...
      //SOMEONE MENTIONED
        ...
        let doneAv
        try {
          doneAv = await new DIG.Triggered().getImage(`${mentioned.avatarURL({ dynamic: false, format: "png" })}?size=4096`)
        } catch (err) {
          errEmbed.setDescription(`${client.emotes.grverify} Tworzenie wiadomości\n${client.emotes.grverify} Wzmianka: ${mentioned}\n${client.emotes.x} Przerabianie avataru`)
          reaction.edit(errEmbed)
          return;
