...
module.exports = {
  ...
  description: 'Zrób kogoś avatar wkurzonym',
  category: 'avatar',
  utilisation: '{prefix}at <wzmianka>',
   ...
   const reaction = await message.lineReplyNoMention(reactionEmbed)
    ...
    try {
      ...
      if (!message.mentions.members.size) {
        ...
        let doneAv
        try {
          doneAv = await new DIG.Triggered().getImage(`${message.author.displayAvatarURL({ dynamic: false, format: "png" })}?size=4096`)
        } catch (err) {
        ...
        //ATTACH AV
        try {
          const attachment = new MessageAttachment(doneAv, "triggered.gif")
      ...
      //SOMEONE MENTIONED
        ...
        let doneAv
        try {
          doneAv = await new DIG.Triggered().getImage(`${mentioned.displayAvatarURL({ dynamic: false, format: "png" })}?size=4096`)
        } catch (err) {
          errEmbed.setDescription(`${client.emotes.grverify} Tworzenie wiadomości\n${client.emotes.grverify} Wzmianka: ${mentioned}\n${client.emotes.x} Przerabianie avataru`)
          reaction.edit(errEmbed)
          return;
