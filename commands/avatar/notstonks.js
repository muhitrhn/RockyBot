...
module.exports = {
    name: "notstonks",
    aliases: ["ans"],
    description: 'Kompletnie nie stonks',
    category: 'avatar',
    utilisation: '{prefix}notstonks <wzmianka>',
    ...
    if (!message.mentions.members.size) {
        ...
        //AVATAR
          ...
          new DIG.NotStonk().getImage(`${message.author.displayAvatarURL({ dynamic: false, format: "png" })}?size=4096`).then(async image => {
    ...
    //SOMEONE MENTIONED
         ...
         if (!mentioned.avatarURL()) {
          ...
          //AVATAR
            ...
            embed.setDescription(`${client.emotes.system}  Użyto komendy: **${message.content}**`)
            new DIG.NotStonk().getImage(`${mentioned.displayAvatarURL({ dynamic: false, format: "png" })}?size=4096`).then(async image => {
              let attachment = new MessageAttachment(image, "NotStonks.png")
              await message.lineReplyNoMention({embed, files: [attachment]})
              if (reaction) await reaction.remove()
            })
