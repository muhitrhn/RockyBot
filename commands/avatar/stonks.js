...
module.exports = {
  ...
  category: 'avatar',
  utilisation: '{prefix}as <wzmianka>',
  async execute(client, message, args, pf, cmd) {
    ...
    try {
      ...
      if (!message.mentions.members.size) {
        ...
        let doneAv
        try {
          doneAv = await new DIG.Stonk().getImage(`${message.author.avatarURL({ dynamic: false, format: "png" })}?size=4096`)
        } catch (err) {
        ...
        //ATTACH AV
        try {
          const attachment = new MessageAttachment(doneAv, "stonks.png")
      ...
      //SOMEONE MENTIONED
        ...
        let doneAv
        try {
          doneAv = await new DIG.Stonk().getImage(`${mentioned.avatarURL({ dynamic: false, format: "png" })}?size=4096`)
        } catch (err) {
          errEmbed.setDescription(`${client.emotes.grverify} Tworzenie wiadomości\n${client.emotes.grverify} Wzmianka: ${mentioned}\n${client.emotes.x} Przerabianie avataru`)
          reaction.edit(errEmbed)
          return;
