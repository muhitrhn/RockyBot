  ...
  //@ts-ignore
	...
	if (!mentioned?.manageable) {
		embed.setTitle(`${config.emotes.warn}  Moja rola jest za niska...`)
			.setDescription(`**...aby zdjąć rolę wyciszenia z [${await mentioned?.user.tag}](https://discord.com/users/${mentioned?.id})**`)
	...
	if (!mentioned.roles.cache.some(role => role.id === data.MutedRole)) {
		embed.setTitle(`${config.emotes.warn}  Użytkownik nie jest wyciszony,...`)
			.setDescription('**...użyj `/mod mute` aby go wyciszyć**')
			.setThumbnail(config.cmds.errorImgs[Math.floor(Math.random() * config.cmds.errorImgs.length)])
			.setColor('#FFC000')
	...
	embed.setTitle(`${config.emotes.siren}  Czy na pewno chcesz odciszyć...`)
  ...
  //@ts-ignore
    ...
    //@ts-ignore
		if (buttonClick.customId === 'cancel') {
			...
			embed.setTitle(`${config.emotes.rverify}  Anulowano odciszanie użytkownika...`)
      ...
      //@ts-ignore
		else if (buttonClick.customId === 'unmute') {
			try {
				await mentioned.roles.remove(role, reasonToProvide)
			} 
			catch (err) {
				embed.setTitle(`${config.emotes.warn}  Nie mogę odciszyć użytkownika...`)
